#!/home/koerstz/opt/miniconda3/envs/take_step_om/bin/python3
# n.b. chmod 755
...
def get_gradient(natoms):
    ...
    for i, line in enumerate(gradient_data[:natoms]):
        line = line.split()
        line = [num.replace("D", "E") for num in line]
        gradient[i, :] = np.asarray(line)
    return gradient
...
def run_xtb(natoms, nderiv, chrg, spin, atomtypes, coords, cpus=1, solvent=None):
    """ """
    ...
    cmd = f"{os.environ['XTB_CMD']} mol.xyz --chrg {chrg} --uhf {spin - 1} --gfn 2 "
    cmd += "--norestart "
    if nderiv == 1:
        cmd += "--grad "
    elif nderiv == 2:
    ...
    print("script called: ", __file__)
    print("with the cmd: ", cmd)
